...
# that can manipulate that data. One type of function that can be defined within a class is called a "method."
...
# This can be useful if you want to create instances of your class in multiple ways, 
# but still have a consistent interface for doing so.
...
#  It creates a new instance of the class "ExampleClass" using the "cls" keyword, 
# and returns the new instance to the caller.
...
#  rather than on a specific instance of the class.
#  They are useful for creating factory methods, alternative constructors, 
...
class Employee:
    ...
    def __init__(self, name, id):
        ...
        self.id=id
    def show(self):
        print(f"The employee name is {self.name} and its id is {self.id} and its company name is {self.companyname}")
    @classmethod  
    def changecompany(cls, changename):
        cls.companyname=changename
emp1=Employee("Maria", 5)
