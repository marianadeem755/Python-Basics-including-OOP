...
# This module is part of the Python Standard Library and is available in all Python installations,
#  making it a convenient and essential tool for a wide range of applications. 
...
#  The returned value is based on the computer's system clock and is affected by time adjustments made by the operating system, such as daylight saving time. Here's an example:

# import time
# print(time.time())
# # Output: 1602299933.233374
# As you can see, the function returns the current time as a floating-point number,
#  which can be used for various purposes, such as measuring the duration of an operation or the elapsed time 
...
# This function can be used to pause the program for a certain period of time, allowing other parts of the program
#  to run, or to synchronize the execution of multiple threads. Here's an example:
...
# The time module in Python provides a set of functions to work with time-related operations, 
# such as timekeeping, formatting, and time conversions.
#  Whether you are writing a script, a library, or an application, the time module is a powerful tool that can
#  help you perform time-related tasks with ease and efficiency. So, if you haven't already, be sure to check out 
...
init=time.time()
UsingWhile()
print(f"The time consuming for while loop is: {time.time()-init}")
print(f"The time taken for running for loop is {t}")
import time
print(4)
time.sleep(2)
print("we have to give gap of 2 sec")
