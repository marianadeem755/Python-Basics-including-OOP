...
# improve the performance of your code. In this repl, we'll take a closer look at the multiprocessing module
#  and its various functions and how they can be used in Python.
...
# multiprocessing.Process(target, args): This function creates a new process that runs the target function
#  with the specified arguments.

# multiprocessing.Pool(processes): This function creates a pool of worker processes that can be used to parallelize
...
def DownloadFile(url, name):
  ...
  open(f"code_with_herry_course/filestodownload/File {name}.jpg", "wb").write(response.content)
...
def func(task):
    print("Multiprocess Pool Task", task)
if __name__=="__main__":
        tasks=[1,2,3,4,5,17,18,19,20,21,22,23,24,25]
...
#  One way to do this is by using a queue. A queue is a data structure that allows data to be inserted 
...
if __name__ == '__main__':
  ...
  p2.start()
  p1.join()
  p2.terminate() # is kai begair function terminate nhi kerai ga and run kerta jai ga

# Using a lock to synchronize access to shared resources
# When working with multiprocessing in python, locks can be used to synchronize access to shared resources among
#  multiple processes. A lock is an object that acts as a semaphore, allowing only one process at a time to execute 
# a critical section of code. The lock is released when the process finishes executing the critical section.


# Conclusion
#  As you can see, the multiprocessing module provides a simple and efficient way to run multiple processes
#  in parallel. Whether you need to create a new process, run a function across multiple input values, 
# communicate data between processes, or synchronize access to shared resources, the multiprocessing module
#  has you covered.

# In conclusion, the multiprocessing module is a powerful tool for parallelizing code in Python.
#  Whether you are a beginner or an experienced Python developer, the multiprocessing module is an essential tool 
